...
public class HttpWebService {
	...
	private DefaultHttpClient _defClient = new DefaultHttpClient();
	...
	protected DefaultHttpClient GetHttpClient() throws Exception {
		if (ignoreCertificateErrors) {
			...
			workAroundReverseDnsBugInHoneycombAndEarlier(_defClient);
		}
	...
	@SuppressWarnings("rawtypes")
	protected InputStream postWS(SoapRequest request, String xml)
			throws Exception {
		...
		if (this._connectionTimeout > 0) {
			HttpConnectionParams.setConnectionTimeout(c.getParams(),
					_connectionTimeout);
		}
	...
	private void workAroundReverseDnsBugInHoneycombAndEarlier(
		...
		client.setHttpRequestRetryHandler(new HttpRequestRetryHandler() {
			...
			public boolean retryRequest(IOException ex, int retry,
				...
				if (ex instanceof NoHttpResponseException) {
					System.out.println("WSClient++ trying for "
		...
		SocketFactory socketFactory = new LayeredSocketFactory() {
			...
			public Socket createSocket(Socket socket, String host, int port,
				...
				SSLSocket ss = (SSLSocket) delegate.createSocket(socket, host,
						port, autoClose);
			...
			private void injectHostname(Socket socket, String host) {
				try {
					...
					System.out.println("WSClient++ "
							+ field.get(socket.getInetAddress()).toString());
					System.out.println("WSClient++ Successfully injected "
							+ host);
